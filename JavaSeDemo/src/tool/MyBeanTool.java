 ...
 * @Description:
 * @author: yusheng
 * @create-date: 2019-10-18 13:35
 **/
public class MyBeanTool {
    ...
    //BeanCopier的类锁，为了线程安全，启动同步机制
    private static Object beanCopierLock = new Object();
    //Converter的类锁，为了线程安全，启动同步机制
    ...
    public static <T extends Serializable> T deepCopy(T srcObject) {
        T cloneObject = null;
        try {
    ...
    private static Converter newConverter(Object src, Object target, final String fields, final boolean fieldCopyFlag) {
        ...
        if (null == converter) {
            //当前线程中没有converter，等待获取类锁，防止弄脏数据
